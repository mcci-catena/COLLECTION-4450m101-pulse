...
#---- project settings -----
readonly ARDUINO_FQBN="mcci:samd:mcci_catena_4450"
...
readonly ARDUINO_SOURCE=sketches/catena4450m101_sensor/catena4450m101_sensor.ino

#---- common code ----
BSP_MCCI=$HOME/.arduino15/packages/mcci
BSP_CORE=$BSP_MCCI/hardware/samd/
LOCAL_BSP_CORE="$(realpath extra/ArduinoCore-samd)"
...
function _errcleanup {
    ...
    rm -f "$OUTPUT"/*.elf "$OUTPUT"/*.bin "$OUTPUT"/*.hex
}
...
if [[ ! -h "$OLD_BSP_CORE" ]] && [[ -d "$OLD_BSP_CORE" ]]; then
    ...
    ln -sf "$LOCAL_BSP_CORE" "$BSP_CORE"/2.3.0
